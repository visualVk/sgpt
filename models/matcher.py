...
class HungarianMatcherHOI(nn.Module):
    ...
    def __init__(self, cost_obj_class: float = 1, cost_verb_class: float = 1, cost_bbox: float = 1,
                 cost_giou: float = 1, cost_joint_bbox: float = 1, cost_joint_class: float = 1):
        super().__init__()
        self.cost_obj_class = cost_obj_class
        self.cost_verb_class = cost_verb_class
        self.cost_bbox = cost_bbox
        self.cost_giou = cost_giou
        self.cost_joint_bbox = cost_joint_bbox
        self.cost_joint_class = cost_joint_class
    ...
    def forward(self, outputs, targets):
        ...
        cost_obj_bbox = torch.cdist(
            out_obj_bbox, tgt_obj_boxes, p=1) * (tgt_obj_boxes != 0).any(dim=1).unsqueeze(0)
        ...
        cost_sub_giou = - \
            generalized_box_iou(box_cxcywh_to_xyxy(
                out_sub_bbox), box_cxcywh_to_xyxy(tgt_sub_boxes))
        ...
        if cost_sub_giou.shape[1] == 0:
            ...
            cost_giou = torch.stack(
                (cost_sub_giou, cost_obj_giou)).max(dim=0)[0]
...
class DualMatcher(nn.Module):

    def __init__(self):
        super().__init__()

    @torch.no_grad()
    def cross_match(self, outputs, targets):
        cross_matrix = []
        for (oi, oj), (ti, tj) in zip(outputs, targets):
            _, indices_oj = oj.sort(dim=-1)
            _, indcies_tj = tj.sort(dim=-1)
            cross_matrix.append((oi[indices_oj], ti[indcies_tj]))
        return cross_matrix

    @torch.no_grad()
    def forward(self, outputs, aug_path):
        cross_matrix = []
        for i in range(aug_path):
            for j in range(i + 1, aug_path):
                output = outputs[i]
                target = outputs[j]
                cross_matrix.append(self.cross_match(output, target))
        return cross_matrix


def build_dual_matcher():
    return DualMatcher()
