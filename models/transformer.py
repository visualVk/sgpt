...
from models.attention import IIA
...
class Transformer(nn.Module):
    ...
    def forward(self, src, mask, query_embed, query_embed_h, query_embed_o, pos_guide_embed, pos_embed):
        # flatten NxCxHxW to HWxNxC
        bs, c, h, w = src.shape
        num_queries = query_embed_h.shape[0]
        src = src.flatten(2).permute(2, 0, 1)
        pos_embed = pos_embed.flatten(2).permute(2, 0, 1)
        query_embed_h = query_embed_h + pos_guide_embed
        query_embed_o = query_embed_h + pos_guide_embed
        ...
        ins_hs = self.decoder(tgt, memory, memory_key_padding_mask=mask,
        ...
        inter_hs = self.decoder(inter_tgt, memory, memory_key_padding_mask=mask,
                                pos=pos_embed, query_pos=ins_guided_embed)
        refined_inter_hs = self.iia(ins_guided_embed, inter_hs)
        refined_inter_hs = refined_inter_hs.transpose(1, 2)

        h_hs = torch.stack([h_hs])
        o_hs = torch.stack([o_hs])
        refined_inter_hs = torch.stack([refined_inter_hs])

        return hs, h_hs, o_hs, refined_inter_hs, memory.permute(1, 2, 0).view(bs, c, h, w)
