    ...
    if N_init < N * H * W:
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            A = torch.sparse.FloatTensor(
                coor, value, torch.Size([B * H * W, B * N]))
        ...
        A = torch.sparse.FloatTensor(
            coor, value, torch.Size([B, H * W, N])).to_dense()
...
def token_downup(target_dict, source_dict):
    ...
    if N_init < T * S:
        # use sparse matrix multiplication
        # Flops: B * N_init * (C+2)
        idx_token_t = idx_token_t + \
            torch.arange(B, device=x_s.device)[:, None] * T
        idx_token_s = idx_token_s + \
            torch.arange(B, device=x_s.device)[:, None] * S
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            A = torch.sparse.FloatTensor(
                coor, weight, torch.Size([B * T, B * S]))
            # normalize the matrix
            all_weight = A.type(
                torch.float32) @ x_s.new_ones(B * S, 1).type(torch.float32) + 1e-6
            weight = weight / all_weight[(idx_token_t).reshape(-1), 0]
            A = torch.sparse.FloatTensor(
                coor, weight, torch.Size([B * T, B * S]))
            # sparse matmul
            x_out = A.type(torch.float32) @ x_s.reshape(B *
                                                        S, C).type(torch.float32)
    else:
        # use dense matrix multiplication
        # Flops: B * T * S * (C+2)
        idx_batch = torch.arange(B, device=x_s.device)[
            :, None].expand(B, N_init)
        coor = torch.stack(
            [idx_batch, idx_token_t, idx_token_s], dim=0).reshape(3, B * N_init)
        weight = weight.detach()  # detach to reduce training time
        # build a matrix with shape [B, T, S]
        A = torch.sparse.FloatTensor(
            coor, weight, torch.Size([B, T, S])).to_dense()
        # normalize the matrix
        A = A / (A.sum(dim=-1, keepdim=True) + 1e-6)
        # dense matmul
        x_out = A @ x_s
...
def vis_tokens(img, token_dict, edge_color=[1.0, 1.0, 1.0], edge_width=1):
    ...
    for i in range(edge_width):
        edge_map = F.conv2d(
            F.pad(idx_map, [1, 1, 1, 1], mode='replicate'), kernel)
        edge_map = (edge_map != 0).max(dim=1, keepdim=True)[0]
        idx_map = idx_map * (~edge_map) + torch.rand(idx_map.shape,
                                                     device=device, dtype=dtype) * edge_map

    edge_color = torch.tensor(edge_color, device=device, dtype=dtype)[
        None, :, None, None]
...
def get_token_density_map(token_dict):
    ...
    token_density = 1 / \
        torch.sparse.FloatTensor(coor, tmp, torch.Size([B, N])).to_dense()
    tmp_dict = token_dict.copy()
