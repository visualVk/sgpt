        ...
        if self.img_set == 'train':
            ...
            for hoi in img_anno['hoi_annotation']:
                ...
                if sub_obj_pair in sub_obj_pairs:
                    ...
                    sub_ids.append(torch.tensor(
                        kept_box_indices.index(sub_obj_pair[0])))
            ...
            target['ids'] = torch.tensor(
                [anno['id'] for anno in img_anno['annotations']], dtype=torch.int64)
            target['img_id'] = int(
        ...
        for img_anno in annotations:
            ...
            for hoi in hois:
                triplet = (self._valid_obj_ids.index(bboxes[hoi['subject_id']]['category_id']),
                           self._valid_obj_ids.index(
                               bboxes[hoi['object_id']]['category_id']),
                           self._valid_verb_ids.index(hoi['category_id']))
                counts[triplet] += 1
        self.rare_triplets = []
...
def build(image_set, args):
    ...
    PATHS = {
        'train': (root / 'images' / 'train2015', root / 'annotations' / 'trainval_hico_kpt.json'),
        'val': (root / 'images' / 'test2015', root / 'annotations' / 'test_hico_kpt.json')
    }
